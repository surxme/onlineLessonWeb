...
//                $file_url = DSFormat($file_url);
    ...
    public function videoUpload(){
        $file = request()->file('file');
        if($file){
            $info = $file->rule('uniqid')->move(ROOT_PATH  . 'public' . DS  . 'static' .DS . 'uploads' . DS . 'videos');
            if ($info) {//上传成功
                $file_name = $info->getFilename();
                $file_url = DS . 'uploads' . DS . 'videos' . DS .$file_name;
    ...
    public function attachUpload(){
        $file = request()->file('file');
        if($file){
            $info = $file->rule('uniqid')->move(ROOT_PATH  . 'public' . DS  . 'static' .DS . 'uploads' . DS . 'attachments');
            if ($info) {//上传成功
                $file_name = $info->getFilename();
                $file_url = DS . 'uploads' . DS . 'attachments' . DS .$file_name;
